...
def main():
  ...
  happy = LpVariable.dicts("happy", [(child.id, factory.id) for child in children.values() for factory in child.factories], lowBound=0, upBound=1, cat='Binary')
  ...
  for country in countries.values():
    ...
    problem += lpSum(happy[child.id, factory.id] for child in country.children for factory in child.factories) >= country.min_gifts
